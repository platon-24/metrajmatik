...
use crate::export::{
    metraj_csv_aktar, metraj_csv_oku, metraj_excel_aktar, metraj_json_kaydet, metraj_json_yukle,
    veri_paketi_kaydet, veri_paketi_yukle, AnalizFoyu,
};
...
use crate::pdf_parser::{
    pdf_metin_cikar, pozlari_ayristir, profil_otomatik_sec, AyristirmaProfili,
};
...
impl MetrajApp {
    ...
    pub(crate) fn autosave_kontrol(&mut self, ctx: &egui::Context) {
        ...
        if let Err(e) = metraj_json_kaydet(&m, &yol) {
            self.hata_mesaji = format!("Otomatik kayıt başarısız: {}", e);
        }
    }
    ...
    pub(crate) fn kitaplari_yenile(&mut self) {
        if let Some(ref db) = self.db {
            if let Ok(k) = db.kitaplari_listele() {
                self.kitaplar = k;
            }
        }
    }
    fn metraj_kalemlerini_tekillestir(&mut self) -> usize {
        ...
        for kalem in self.metraj_kalemleri.drain(..) {
            // Aynı poz farklı imalat/kitap/gruplarda meşru olarak tekrarlanabilir.
            // Yalnız gerçekten aynı kalıcı kimliğe sahip yinelenmiş kayıt birleşir.
            if let Some(mevcut) = tekil
                .iter_mut()
                .find(|m| !kalem.id.is_empty() && m.id == kalem.id)
            {
    ...
    pub(crate) fn pozlar_tablosu_yenile(&mut self) {
        ...
        let kitap_id = match self.pozlar_yuklu_kitap_id {
            Some(id) => id,
            None => return,
        };
            ...
            if (!poz_gibi || sonuc.len() < 20) && sorgu.split_whitespace().all(|t| !t.is_empty()) {
                if let Ok(pozlar) = db.tam_metin_ara(sorgu, kid) {
                    for poz in pozlar {
                        if !sonuc
                            .iter()
                            .any(|p| p.poz_no == poz.poz_no && p.kitap_id == poz.kitap_id)
                        {
                            sonuc.push(poz);
                        }
                    }
    ...
    pub(crate) fn poz_no_ara(&mut self) {
        ...
        if let Some(ref db) = self.db {
            let kid = self.secili_kitap.as_ref().map(|k| k.id);
            if let Ok(s) = db.poz_no_ara(&self.poz_arama_metni, kid) {
                self.arama_sonuclari = s;
            }
        }
    }
    pub(crate) fn aciklama_ara(&mut self) {
        if let Some(ref db) = self.db {
            let kid = self.secili_kitap.as_ref().map(|k| k.id);
            if let Ok(s) = db.tam_metin_ara(&self.aciklama_arama_metni, kid) {
                self.arama_sonuclari = s;
            }
        }
    }
    ...
    pub(crate) fn kalem_ekle(&mut self) {
        ...
        if self.metraj_kalemleri.iter().any(|k| k.poz_no == poz.poz_no) {
            self.basarili_mesaj = format!(
                "{} zaten listede var. Miktarını düzenlemek için satıra tıklayın.",
                poz.poz_no
            );
            self.hata_mesaji.clear();
            return;
        ...
        self.metraj_kalemleri
            .sort_by(|a, b| a.poz_no.cmp(&b.poz_no));
        self.aktif_grubu_senkronize();
        self.degisiklik_var = true;
    ...
    pub(crate) fn nakliye_kalem_ekle(&mut self, poz: &Poz, miktar: f64, mesafe: f64) {
        ...
        self.hata_mesaji.clear();
        self.basarili_mesaj = format!(
            "Nakliye eklendi: {} ({:.2} taşıma miktarı).",
            poz.poz_no, miktar
        );
    }
    ...
    pub(crate) fn kurum_disa_aktar_diyalog(&mut self, kitap_id: i64) {
        ...
        if let Some(d) = rfd::FileDialog::new()
            .add_filter("Metrajmatik Veri Paketi", &["mvp"])
            .set_file_name(format!("{}.mvp", paket.kurum))
            .save_file()
        {
            match veri_paketi_kaydet(&paket, &d) {
                Ok(()) => {
                    self.basarili_mesaj =
                        format!("Veri paketi: {} ({} poz)", d.display(), paket.pozlar.len())
                }
    ...
    pub(crate) fn kurum_ice_aktar_diyalog(&mut self) {
        let dosya = match rfd::FileDialog::new()
            .add_filter("Veri Paketi", &["mvp", "json"])
            .pick_file()
        {
            Some(d) => d,
            None => return,
        };
        let paket = match veri_paketi_yukle(&dosya) {
            Ok(p) => p,
            Err(e) => {
                self.hata_mesaji = e;
                return;
        ...
        match sonuc {
            Some(Ok((_, n))) => {
                self.basarili_mesaj = format!("'{}' paketi içe alındı ({} poz).", paket.kurum, n)
            }
            Some(Err(e)) => {
                self.hata_mesaji = format!("{}", e);
                return;
        ...
        self.poz_sayisi = self
            .db
            .as_ref()
            .and_then(|db| db.poz_sayisi().ok())
            .unwrap_or(self.poz_sayisi);
    ...
    pub(crate) fn veritabani_yedekle_diyalog(&mut self) {
        let db = match self.db.as_ref() {
            Some(d) => d,
            None => {
                self.hata_mesaji = "Veritabanı açık değil!".into();
                return;
            }
        };
        let damga: String = krono_tarih()
            .chars()
            .map(|c| if c.is_ascii_digit() { c } else { '-' })
            .collect();
        let varsayilan = format!("metrajmatik_yedek_{}.db", damga);
        if let Some(d) = rfd::FileDialog::new()
            .add_filter("Metrajmatik Yedek", &["db"])
            .set_file_name(&varsayilan)
            .save_file()
        {
            match db.yedekle(&d) {
    ...
    pub(crate) fn veritabani_geri_yukle_diyalog(&mut self) {
        let kaynak = match rfd::FileDialog::new()
            .add_filter("Metrajmatik Yedek", &["db"])
            .pick_file()
        {
            Some(d) => d,
        ...
        // Adayı salt-okunur aç; doğrulama sırasında dosyaya tablo/pragma yazılmaz.
        ...
        if std::fs::canonicalize(&kaynak).ok() == std::fs::canonicalize(&guvenlik).ok() {
            guvenlik = super::veri_yolu("metrajmatik_restore_oncesi_yeni.db");
        }
        let db = match self.db.as_ref() {
            Some(db) => db,
            None => {
                self.hata_mesaji = "Canlı veritabanı açık değil; geri yükleme iptal edildi.".into();
                return;
            }
        };
        if let Err(e) = db.yedekle(&guvenlik) {
            self.hata_mesaji = format!(
                "Güvenlik yedeği alınamadığı için geri yükleme iptal edildi: {}",
                e
            );
            return;
        }

        let gecici = hedef.with_extension("restore.tmp");
        let eski = hedef.with_extension("restore-old.db");
        let _ = std::fs::remove_file(&gecici);
        let _ = std::fs::remove_file(&eski);
        if let Err(e) = std::fs::copy(&kaynak, &gecici) {
            self.hata_mesaji = format!("Yedek geçici alana kopyalanamadı: {}", e);
            return;
        }

        // Canlı bağlantıyı kapat; son bağlantı kapanınca WAL checkpoint edilir.
        self.db = None;
        ...
        if hedef.exists() {
            if let Err(e) = std::fs::rename(&hedef, &eski) {
                self.hata_mesaji = format!("Mevcut veritabanı güvenli alana taşınamadı: {}", e);
                self.db = crate::database::Veritabani::ac(&hedef).ok();
                let _ = std::fs::remove_file(&gecici);
                return;
            }
        }
        if let Err(e) = std::fs::rename(&gecici, &hedef) {
            let _ = std::fs::rename(&eski, &hedef);
            self.db = crate::database::Veritabani::ac(&hedef).ok();
            self.hata_mesaji = format!("Geri yükleme dosyası etkinleştirilemedi: {}", e);
            return;
        }
        ...
        match crate::database::Veritabani::ac(&hedef) {
            Ok(vt) => {
                ...
                self.kitaplari_yenile();
                self.basarili_mesaj = format!(
                    "Yedek geri yüklendi. İşlem öncesi güvenlik kopyası: {}",
                    guvenlik.display()
                );
            }
            Err(e) => {
                let _ = std::fs::remove_file(&hedef);
                if std::fs::rename(&eski, &hedef).is_err() {
                    let _ = std::fs::copy(&guvenlik, &hedef);
                }
                self.db = crate::database::Veritabani::ac(&hedef).ok();
                self.hata_mesaji = format!(
                    "Geri yüklenen veritabanı açılamadı; önceki veri geri alındı: {}",
                    e
                );
            }
        }
    ...
    pub(crate) fn kategorileri_yukle(&mut self) {
        if let Some(ref db) = self.db {
            let kid = self.secili_kitap.as_ref().map(|k| k.id);
            if let Ok(k) = db.kategoriler(kid) {
                self.kategoriler = k;
            }
        }
    }
    pub(crate) fn kategori_filtrele(&mut self) {
        if let Some(ref db) = self.db {
            let kid = self.secili_kitap.as_ref().map(|k| k.id);
            if let Ok(t) = db.tum_pozlar(kid) {
                self.kategori_pozlar = t
                    .into_iter()
    ...
    // ==================== DOSYA ====================
    pub(crate) fn pdf_sec_ve_yukle(&mut self) {
        if let Some(y) = rfd::FileDialog::new()
            .add_filter("PDF", &["pdf"])
            .pick_file()
        {
            self.pdf_yukle(y);
    ...
    pub(crate) fn pdf_yukle(&mut self, pdf_yolu: PathBuf) {
        ...
        self.pdf_durumu = "PDF okunuyor...".to_string();
        match pdf_metin_cikar(&pdf_yolu) {
            Ok(metin) => {
                ...
                if let Some(ref db) = self.db {
                    match db.pozlari_yukle(kitap.id, yil, ay, &pozlar) {
                        Ok(sayi) => {
            ...
            Err(e) => self.hata_mesaji = e.to_string(),
        }
    ...
    pub(crate) fn metraj_kaydet(&mut self) {
        ...
        let m = self.proje_olustur();
        let hedef = if let Some(yol) = self.mevcut_dosya_yolu.clone() {
            Some(yol)
        } else {
            rfd::FileDialog::new()
        ...
        match metraj_json_kaydet(&m, &hedef) {
            Ok(()) => {
                ...
                self.basarili_mesaj = format!("Kaydedildi: {}", hedef.display());
                self.hata_mesaji.clear();
            }
            Err(e) => self.hata_mesaji = e,
        }
    ...
    pub(crate) fn metraj_yukle_diyalog(&mut self) {
        ...
        {
            if self.degisiklik_var {
                self.bekleyen_acma_yolu = Some(d);
            } else {
    ...
    /// Kirli projeyi değiştirecek dosya açma veya uygulama kapatma isteğinde
    /// kaydet/kaydetme/vazgeç kararını kullanıcıdan alır.
    pub(crate) fn render_kaydetme_onaylari(&mut self, ctx: &egui::Context) {
        if let Some(yol) = self.bekleyen_acma_yolu.clone() {
            let mut kaydet_ac = false;
            let mut kaydetmeden_ac = false;
            ...
            egui::Window::new("Kaydedilmemiş değişiklikler")
                ...
                .show(ctx, |ui| {
                    ui.label("Mevcut projedeki değişiklikler kaydedilmedi.");
                    ui.label(format!("Açılacak dosya: {}", yol.display()));
                    ui.add_space(8.0);
                    ui.horizontal(|ui| {
                        if crate::tema::basari_buton(ui, "Kaydet ve Aç").clicked() {
                            kaydet_ac = true;
                        }
                        if crate::tema::tehlike_buton(ui, "Kaydetmeden Aç").clicked() {
                            kaydetmeden_ac = true;
                        }
            ...
            if kaydet_ac {
                self.metraj_kaydet();
                if !self.degisiklik_var {
                    self.bekleyen_acma_yolu = None;
                    self.metraj_dosyadan_yukle(&yol, true);
                }
            } else if kaydetmeden_ac {
                self.bekleyen_acma_yolu = None;
                self.metraj_dosyadan_yukle(&yol, true);
            } else if vazgec {
                self.bekleyen_acma_yolu = None;
        ...
        if self.kapanis_onayi {
            let mut kaydet_kapat = false;
            let mut kaydetmeden_kapat = false;
            let mut vazgec = false;
            egui::Window::new("Uygulamayı kapat")
                .collapsible(false)
                .resizable(false)
                .anchor(egui::Align2::CENTER_CENTER, [0.0, 0.0])
                .show(ctx, |ui| {
                    ui.label("Kaydedilmemiş değişiklikler var.");
                    ui.add_space(8.0);
                    ui.horizontal(|ui| {
                        if crate::tema::basari_buton(ui, "Kaydet ve Kapat").clicked() {
                            kaydet_kapat = true;
                        }
                        if crate::tema::tehlike_buton(ui, "Kaydetmeden Kapat").clicked() {
                            kaydetmeden_kapat = true;
                        }
                        if ui.button("Vazgeç").clicked() {
                            vazgec = true;
                        }
            ...
            Ok(mut m) => {
                ...
                if dosya_olarak {
                    ...
                    self.basarili_mesaj = if birlesen > 0 {
                        format!(
                            "Açıldı: {} ({} yinelenen poz birleştirildi)",
                            d.display(),
                            birlesen
                        )
                    } else {
                    ...
                    self.degisiklik_var = true;
                    self.basarili_mesaj =
                        "Otomatik kayıttan kurtarıldı. Lütfen 'Kaydet' ile kalıcı hale getirin."
                            .into();
                }
            }
            Err(e) => self.hata_mesaji = e.to_string(),
    ...
    pub(crate) fn metraj_excel_diyalog(&mut self) {
        ...
        if let Some(ref db) = self.db {
            let mut gorulen: std::collections::HashSet<(i64, String)> =
            ...
            .save_file()
        {
            match metraj_excel_aktar(&m, &analizler, &d) {
                Ok(()) => {
                    self.basarili_mesaj = format!("Excel: {}", d.display());
                }
                Err(e) => self.hata_mesaji = e.to_string(),
            }
        }
    ...
    pub(crate) fn teklif_cetveli_diyalog(&mut self, dolu: bool) {
        ...
        if let Some(d) = rfd::FileDialog::new()
            .add_filter("Excel", &["xlsx"])
            .set_file_name(format!("{} - {}.xlsx", self.metraj_adi, ek))
            .save_file()
        {
    ...
    pub(crate) fn metraj_csv_diyalog(&mut self) {
        ...
        if let Some(d) = rfd::FileDialog::new()
            .add_filter("CSV", &["csv"])
            .set_file_name(format!("{}.csv", self.metraj_adi))
            .save_file()
        {
    ...
    pub(crate) fn metraj_csv_ice_aktar_diyalog(&mut self) {
        ...
        let satirlar = match metraj_csv_oku(&dosya) {
            Ok(s) => s,
            Err(e) => {
                self.hata_mesaji = e;
                return;
        ...
        if let Some(ref db) = self.db {
            for (poz_no, miktar, imalat) in satirlar {
                if self.metraj_kalemleri.iter().any(|k| k.poz_no == poz_no) {
                    continue;
                }
                match db.poz_getir(&poz_no, kid) {
                    Ok(Some(poz)) => {
        ...
        self.basarili_mesaj = if bulunamayan.is_empty() {
            ...
            format!(
                "CSV'den {} kalem eklendi. {} poz bulunamadı: {}",
                eklenen,
                bulunamayan.len(),
                ornek.join(", ")
            )
    ...
    pub(crate) fn fiyatlari_guncelle(&mut self) {
        ...
        fn agaci_gez(gruplar: &mut [IsGrubu], f: &mut dyn FnMut(&mut MetrajKalemi)) {
            for g in gruplar.iter_mut() {
                for kalem in g.kalemler.iter_mut() {
                    f(kalem);
                }
        ...
        if self.fiyat_guncelle_endeks_mod {
            ...
            if temel <= 0.0 {
                self.hata_mesaji = "Temel endeks sıfırdan büyük olmalı.".into();
                return;
            }
            ...
            let hedef = match self.fiyat_guncelle_hedef.clone() {
                Some(k) => k,
                None => {
                    self.hata_mesaji = "Lütfen hedef kurum seçin.".into();
                    return;
                }
            };
            ...
            if let Some(ref db) = self.db {
                ...
                let mut kalem_guncelle = |kalem: &mut MetrajKalemi| {
                    let yeni = if en_son {
                        db.poz_getir(&kalem.poz_no, Some(hedef.id))
                            .ok()
                            .flatten()
                            .and_then(|p| p.fiyat)
                    } else {
                        db.poz_fiyat_asof(hedef.id, &kalem.poz_no, ty, ta)
                            .ok()
                            .flatten()
                    };
                    if let Some(f) = yeni {
                        kalem.birim_fiyat = f;
                        kalem.kitap_adi = if en_son {
                            hedef.ad.clone()
                        } else {
                            format!("{} ({}/{})", hedef.ad, ta, ty)
                        };
                        kalem.tutar_guncelle();
                        guncellenen += 1;
                    } else {
