...
impl MetrajApp {
    ...
    pub(crate) fn render_analiz_popup(&mut self, ctx: &egui::Context) {
        ...
        .show(ctx, |ui| {
                ...
                egui::Grid::new("analiz_grid")
                    ...
                    .show(ui, |ui| {
                        ...
                        for (i, satir) in self.analiz_girdileri.iter_mut().enumerate() {
                            ...
                            ui.add(
                                TextEdit::singleline(&mut satir.miktar_metni).desired_width(62.0),
                            );
                            let tutar =
                                sayi_oku(&satir.miktar_metni).unwrap_or(0.0) * satir.birim_fiyat;
                            ui.label(
            ...
            ui.horizontal(|ui| {
                if tema::basari_ikonlu_buton(ui, tema::ikon::ONAY, "Poz Fiyatı Yap")
                    .on_hover_text("Analizi kaydeder ve sonucu pozun birim fiyatı yapar")
                    .clicked()
                {
                    poz_fiyati_yap = true;
                }
                if tema::ikincil_ikonlu_buton(ui, tema::ikon::KAYDET, "Yalnız Analizi Kaydet")
                    .clicked()
                {
                    yalniz_kaydet = true;
                }
